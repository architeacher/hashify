...
type (
	...
	Error struct {
		...
		// e.g. Bus.Publish()
		Op Operation
		// Kind of the error such as subscriber not found
...
func (k Kind) String() string {
	switch k {
	case Invalid:
		return "invalid input is provided"
	case NotFound:
		return "not found"
	case MinLength:
		return "not matching minimum length constraint"
	case Failure:
		return "could not perform operation on the provided data %s"
	case Panic:
		return "panic"
	}
